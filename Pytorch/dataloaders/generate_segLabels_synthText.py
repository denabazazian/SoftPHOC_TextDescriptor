...
char2int = {'a': 1, 'A': 1, 'b': 2, 'B': 2, 'c': 3, 'C': 3, 'd': 4, 'D': 4, 'e': 5, 'E': 5, 'f': 6, 'F': 6, 'g': 7,
            ...
            '"': 37, '\\': 37, '|':37, '~':37}   #'\xc2': 37, '\xb4': 37, ' ': 37, '\xc3': 37, '\x89': 37}  # '\':37
numClasses = 38
...
def synthesize(word, width, height, vis=False):    
    ...
    chars = [char2int.get(x, 37) for x in word]
...
def process_image(gtFile,imgFile,resizing=True):
    # tt = time.time()
    #print('Image File is: {}'.format(imgFile))
    img = cv2.imread(imgFile)   
    try:
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    except:
        print("corrupt image is: {}".format(imgFile))    
    cv_size = lambda img: tuple(img.shape[1::-1])    
    width_main, height_main = cv_size(img)
    # try:
    #     img = Image.open(imgFile)
    #     width_main, height_main = img.size
    # except:
    #     print("corrupt image is: {}".format(imgFile))
    ...
    gt = open(gtFile).read()
    ## C Comment --> gt = gt_dict[gtFile]  --> define a dictionary for all the gt files at the beginning of the process, 
    ##                                          for each item have the key as the path and define another dictionary inside
    ##                                          for each transcription have the coordinates
    ...
    for GTword in range(numbWords):
        if (len(gt[GTword]) > 0 and gt[GTword].split(',')[-1].strip() != '###'):
            ...
            transcription = unicodedata.normalize('NFKD', transcription).encode('ASCII', 'ignore').decode()
            ...
            if (width>1 and height>1 and width<400 and height<400):
...
def generate_segmentation_softPHOC_labels(gt_file,img_file):
    ...
    #print("gt_file is: {}".format(gt_file))
    ...
    img, softphoc, b_mask, enlarged_b_mask, width_img, height_img = process_image(gt_file,img_file,resizing=True)

    return img, softphoc, b_mask, enlarged_b_mask
