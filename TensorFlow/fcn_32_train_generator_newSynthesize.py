    ...
    while True:
        ...
        for img_name in imgnames:
            try:
                ...
                # print 'process_image: ' + str(time.time() - t)
...
gtPath = '/datasets/synthtext/gt/gt_word_polygon/'
...
img_path = '/datasets/synthtext/input_images/'
index_training_img = '/datasets/synthtext/gt/img_idx_shuffle_maxIoUZero_training.txt'
index_testing_img = '/datasets/synthtext/gt/img_idx_shuffle_maxIoUZero_testing.txt'
...
training_dataset = training_dataset.prefetch(50)
...
sys.path.append("./slim/")  # for reading the net
sys.path.append("./models/")
sys.path.append("./softPHOC_utils/")
...
log_folder = './training_generator_newsynthesize_log/' + str(time.strftime("%Y:%m:%d:%H:%M:%S")) + '_(' + str(w1) + ',' + str(
    w2) + ',' + str(w3) + ')' + '_lr_' + str(lr)

if not os.path.exists(log_folder):
    os.makedirs(log_folder)

# log_folder = './log/log_2'   #$ python -m tensorflow.tensorboard --logdir=.            #$ tensorboard --logdir=.
...
with tf.variable_scope("adam_vars"):
    ...
    # train_step = tf.train.AdamOptimizer(learning_rate=lr).minimize(total_loss, var_list=tf.trainable_variables()[-12:]) #finetune starting from conv5
...
with tf.Session(config=config) as sess:
    ...
    #saver.restore(sess, tf.train.latest_checkpoint('/tensorflow/phoc/phocscene/training_generator_newsynthesize_65K'))

    checkpoint_interval = 500
    dump_data_interval = 15000000
    train_iters = 700002
    test_interval = 1500
    ...
    # start training
    for i in xrange(65001, train_iters):
        ...
        if i % summaryWrite_interval == 0:
            ...
            print 'took: ' + str(time.time() - ttttt)

        # save a checkpoint every checkpoint_interval iterations
        if i % checkpoint_interval == 0:
            save_path = saver.save(sess, "./training_generator_newsynthesize/model_fcn32s.ckpt")
        ...
        if i % dump_data_interval == 0:
            ...
            # sio.savemat('./batch/batch_2/batch' + str(i) + '.mat',
            #             {
