...
# python dtw_houghTransform_extractBBX.py /softPHOC_synthText/word_spotting/data/icdar_ch4_testSet/img_*.jpg
...
def perspectiveTransfer(img, crd):
    width = np.max([(abs(crd[1, 0] - crd[0, 0])), (abs(crd[2, 0] - crd[3, 0]))],
                   axis=0)  # (top_left_X, top_right_X), (bottom_left_X, bottom_right_X)
    height = np.max([(abs(crd[0, 1] - crd[3, 1])), (abs(crd[1, 1] - crd[2, 1]))],
                    axis=0)  # (top_left_Y, top_right_Y), (bottom_left_X, bottom_right_Y)
...
def get_houghTransform_points(img, query, probabilities_np, img_width, img_height, max_lines=5,
    ...
    for i_ in range(1, len(chars)):
        partial_prods[i_ - 1] = np.prod(probabilities_np.squeeze()[:, :, [chars[i_ - 1], chars[i_]]], axis=-1,
                                        keepdims=True)

    #for cc in range(len(chars)-1) :
    #    print len(np.prod(probabilities_np.squeeze()[:, :, [chars[cc], chars[cc+1]]], axis=-1, keepdims=True))
    # for cc in range(len(chars)-1) :
    #     plt.imshow(np.prod(probabilities_np.squeeze()[:, :, [chars[cc], chars[cc+1]]], axis=-1, keepdims=True))
    #     plt.imshow(img,alpha=0.5)
    #     plt.title(('%s,%s')%(chars[i_ - 1], chars[i_]))
    #     plt.show()
...
def get_points_lines(houghLines_points, probabilities_np):
    ...
    return pred_points_array, line_points, angle


def get_axis_oriented_proposals(polygone_proposals, logits):
    numProposals = len(polygone_proposals)
    bboxes = np.empty([numProposals, 4]).astype(np.float).astype(
        np.int)  # TODO: this is ugly, had some problems casting to int
...
def box_line_overlap(bb, line_points):
    ...
    overlapping_pixels = len(
        np.where(np.array([cv2.pointPolygonTest(bb_arr, tuple(x), False) for x in line_points]) == 1)[0])
...
if __name__ == '__main__':
    ...
    # img_bbx_path = "./wordspotting/results/dtw/dtw_houghTransform_img_lines/"
    res_path = "./wordspotting/results/dtw/dtw_houghLines_bbx_onlysynth/"
    ...
    similarity_type = 4  # CrossEntropy
    ...
    with tf.Session() as sess:
        ...
        for image_filename in sys.argv[1:]:
            ...
            image_np, pred_np, probabilities_np, logits = sess.run(
                [image_tensor, pred, probabilities, upsampled_logit_resized], feed_dict=feed_dict_to_use)
            ...
            for nqw in range(0, numQWords):
                ...
                if (len(voc[nqw]) > 0 and voc[nqw].split(',')[-1].strip() != '###'):
                    ...
                    st = time.time()
                    houghTransform_proposals = get_houghTransform_points(img, qword, probabilities_np, img_width,
                                                                         img_height, max_lines=5,
                                                                         USE_PROBABILITIES=False)
                    ...
                    softphoc_query = synthesize(number_of_classes, qword, width=target_size[0], height=target_size[1],
                                                vis=False)
                    ...
                    if similarity_type == 0:  # cosine distance
                        ...
                        resized_preds = [(cv2.resize(x, (target_size[0], target_size[1]))) for x in
                                         softphoc_proposal_preds]
                        similarities = (
                        [np.sum(np.minimum(x, softphoc_query) / np.maximum(x, softphoc_query)) / len(qword) for x in
                         resized_preds])

                    elif similarity_type == 4:  # dtw
                        #similarities_vertical = []
                        #similarities_horizontal = []
                        similarities = []
                        #line_ids_vertical = []
                        #line_ids_horiziontal = []
                        line_ids = []
                        line_angle = []
                        # costs = []
                        # accs = []
                        # paths = []
                        im = np.copy(img)
                        for line_id, line in enumerate(houghTransform_proposals):
                            cv2.line(im, (line[0], line[1]), (line[2], line[3]), (0, 255, 0), 2)
                            pred_points_array, _, angle = get_points_lines(line, probabilities_np)
                            ...
                            #     similarities_horizontal.append(-dist)
                            #     line_ids_horiziontal.append(line_id)
                            # elif (90 - angle_margin) <= abs(angle) <= (90 + angle_margin):
                            #     similarities_vertical.append(-dist)
                            #     line_ids_vertical.append(line_id)
                            # else:
                            #     similarities.append(-dist)
                            #     line_ids.append(similarities)
                    ...
                    im = np.copy(img)
                    #sorted_idx_vertical = np.argsort(similarities_vertical)
                    #sorted_idx_horizontal = np.argsort(similarities_horizontal)
                    sorted_idx = np.argsort(similarities)
                    ### sorted_paths = [paths[x] for x in sorted_idx]
                    #print "len sim horizontal:"
                    #print len(similarities_horizontal)
                    #print "len sim vertical:"
                    #print len(similarities_vertical)
                    print "len sim:"
                    print len(similarities)

                    if take_always_argmax:
                        idx_vertical = sorted_idx_vertical[-1]
                        idx_horizontal = sorted_idx_horizontal[-1]
                    else:
                        # idx = sorted_idx[-query_dict[qword]]

                        # if ((len(similarities_horizontal) !=0) and (len(similarities_vertical) != 0)):
                        #     idx_horizontal = sorted_idx_horizontal[np.maximum(-query_dict[qword], -len(sorted_idx_horizontal))]
                        #     res_horizontal = houghTransform_proposals[idx_horizontal]
                        #     cv2.line(im, (res_horizontal[0], res_horizontal[1]), (res_horizontal[2], res_horizontal[3]), (0, 255, 0), 2)
                        #     idx_vertical = sorted_idx_vertical[np.maximum(-query_dict[qword], -len(sorted_idx_vertical))]
                        #     res_vertical = houghTransform_proposals[idx_vertical]
                        #     cv2.line(im, (res_vertical[0], res_vertical[1]), (res_vertical[2], res_vertical[3]), (0, 255, 0), 2)

                        # if len(similarities_vertical) != 0:
                        #     idx_vertical = line_ids_vertical[sorted_idx_vertical[np.maximum(-query_dict[qword], -len(sorted_idx_vertical))]]
                        #     res_vertical = houghTransform_proposals[idx_vertical]
                        #     cv2.line(im, (res_vertical[0], res_vertical[1]), (res_vertical[2], res_vertical[3]), (0, 0, 255), 2)
                        #     # cv2.putText(im, qword, (res[0], res[1]), cv2.FONT_HERSHEY_PLAIN, 1, (0,0, 255), 2)

                        # if len(similarities_horizontal) != 0:
                        #     idx_horizontal = line_ids_horiziontal[sorted_idx_horizontal[np.maximum(-query_dict[qword], -len(sorted_idx_horizontal))]]
                        #     res_horizontal = houghTransform_proposals[idx_horizontal]
                        #     cv2.line(im, (res_horizontal[0], res_horizontal[1]), (res_horizontal[2], res_horizontal[3]), (0, 255, 0), 2)

                        # if ((len(similarities_vertical) == 0) and (len(similarities_horizontal) != 0)):
                        #     pt1 = ((res_horizontal[0] + res_horizontal[2]) / 2,
                        #                 np.min([res_horizontal[1], res_horizontal[3]], axis=0) - ((abs(res_horizontal[0] - res_horizontal[2])) / len(qword)))
                        #     pt2 = ((res_horizontal[0] + res_horizontal[2]) / 2,
                        #            np.max([res_horizontal[1], res_horizontal[3]], axis=0) + ((abs(res_horizontal[0] - res_horizontal[2])) / len(qword)))
                        #     cv2.line(im, pt1, pt2, (255, 0, 0), 2)

                        # if ((len(similarities_vertical) != 0) and (len(similarities_horizontal) == 0)):
                        #     pt1 = (np.min([res_vertical[0], res_vertical[2]], axis=0) + abs(res_vertical[1] - res_vertical[3]) * len(qword),
                        #            (res_vertical[1] + res_vertical[3]) / 2)
                        #     pt2 = (np.min([res_vertical[0], res_vertical[2]], axis=0) - abs(res_vertical[1] - res_vertical[3]) * len(qword),
                        #            (res_vertical[1] + res_vertical[3]) / 2)
                        #     cv2.line(im, pt1, pt2, (255, 255, 0), 2)

                        # if ((len(similarities_vertical) == 0) and (len(similarities_horizontal) == 0) and (len(similarities) != 0)):
                        idx = line_ids[sorted_idx[np.maximum(-query_dict[qword], -len(sorted_idx))]]
                        res = houghTransform_proposals[idx]
                        cv2.line(im, (res[0], res[1]), (res[2], res[3]),(0, 255, 0), 2)        #(0, 255, 255), 2)
                        ...
                        angle_bestLine = line_angle[idx]
                        print "angle is:"
                        print angle_bestLine
                        angle_margin = 45.050
                        if abs(angle_bestLine) <= angle_margin or abs(angle_bestLine) >= (180 - angle_margin): ### the best line is horizontal oriented and we should fine the vertical line
                            res_horizontal = houghTransform_proposals[idx]
                            # pt1 = ((res_horizontal[0] + res_horizontal[2]) / 2,
                            #       np.min([res_horizontal[1], res_horizontal[3]], axis=0) - ((abs(res_horizontal[0] - res_horizontal[2])) / len(qword)))
                            # pt2 = ((res_horizontal[0] + res_horizontal[2]) / 2,
                            #     np.max([res_horizontal[1], res_horizontal[3]], axis=0) + ((abs(res_horizontal[0] - res_horizontal[2])) / len(qword)))
                            # cv2.line(im, pt1, pt2, (0, 0, 255), 2)     #(255, 0, 0), 2)
                            # res_bbx = [res_horizontal[0],res_horizontal[1],pt1[0],pt1[1],res_horizontal[2],res_horizontal[3],pt2[0],pt2[1]]

                            # put all the lines from left to right
                            if res_horizontal[0] > res_horizontal[2]:
                                res_horizontal = [res_horizontal[x] for x in [2, 3, 0, 1]]

                            offset = ((abs(res_horizontal[0] - res_horizontal[2])) / len(qword))
                            p1 = [res_horizontal[0], res_horizontal[1]]
                            p1[1] -= offset
                            p2 = [res_horizontal[2], res_horizontal[3]]
                            p2[1] -= offset
                            p3 = [res_horizontal[2], res_horizontal[3]]
                            p3[1] += offset
                            p4 = [res_horizontal[0], res_horizontal[1]]
                            p4[1] += offset
                            # plt.imshow(im)
                            # plt.show()
                            res_bbx = [p1, p2, p3, p4]
                            # cv2.line(im, tuple(p1), tuple(p2), (0, 0, 255), 2)  # (255, 0, 0), 2)
                            # cv2.line(im, tuple(p2), tuple(p3), (0, 0, 255), 2)  # (255, 0, 0), 2)
                            # cv2.line(im, tuple(p3), tuple(p4), (0, 0, 255), 2)  # (255, 0, 0), 2)
                            # cv2.line(im, tuple(p4), tuple(p1), (0, 0, 255), 2)  # (255, 0, 0), 2)
                            # plt.imshow(im)
                            # plt.title(qword)
                            # plt.show()

                        elif (90 - angle_margin) <= abs(angle) <= (90 + angle_margin):
                            res_vertical = houghTransform_proposals[idx]
                            # pt1 = (np.min([res_vertical[0], res_vertical[2]], axis=0) + abs(res_vertical[1] - res_vertical[3]) * len(qword)/4,
                            #        (res_vertical[1] + res_vertical[3]) / 2)
                            # pt2 = (np.min([res_vertical[0], res_vertical[2]], axis=0) - abs(res_vertical[1] - res_vertical[3]) * len(qword)/4,
                            #        (res_vertical[1] + res_vertical[3]) / 2)
                            # cv2.line(im, pt1, pt2, (255, 255, 0), 2)
                            # res_bbx = [res_vertical[0],res_vertical[1],pt1[0],pt1[1],res_vertical[2],res_vertical[3],pt2[0],pt2[1]]

                            if res_vertical[1] > res_vertical[3]:
                                res_vertical = [res_vertical[x] for x in [2, 3, 0, 1]]

                            offset = ((abs(res_vertical[1] - res_vertical[3])) * len(qword)/4)
                            p1 = [res_vertical[0], res_vertical[1]]
                            p1[0] -= offset
                            p2 = [res_vertical[0], res_vertical[1]]
                            p2[0] += offset
                            p3 = [res_vertical[2], res_vertical[3]]
                            p3[0] += offset
                            p4 = [res_vertical[2], res_vertical[3]]
                            p4[0] -= offset
                            # plt.imshow(im)
                            # plt.show()
                            res_bbx = [p1, p2, p3, p4]
                            ...
                            print "the angle is not defined"
                            res_bbx = [[res[0],res[1]],[res[2],res[1]],[res[2],res[3]],[res[0],res[3]]]
                            res_bbx_ = [res[0], res[1], res[2], res[1], res[2], res[3], res[0], res[3]]
                            try:
                                validate_clockwise_points(res_bbx_)
                            except:
                                res_ = [min(res[0], res[2]), min(res[1], res[3]), max(res[0], res[2]),
                                        max(res[1], res[3])]
                                res_bbx = [[res_[2], res_[3]], [res_[0], res_[3]], [res_[0], res_[1]], [res_[2], res_[1]]]
                    ...
                    # try:
                    #     sim_vertical = similarities_vertical[idx]
                    #     sim_horizontal = similarities_horizontal[idx]
                    # except:
                    #     sim_vertical  = similarities_vertical
                    #     sim_horizontal = similarities_horizontal
                    ...
                    # _,res_line_points,_ =  get_points_lines(res_line, probabilities_np)
                    ...
                    # pts_res = np.array([[res_bbx[0],res_bbx[1]], [res_bbx[2],res_bbx[3]] , [res_bbx[4],res_bbx[5]] , [res_bbx[6],res_bbx[7]]])
                    # pts_res = pts_res.reshape((-1,1,2))
                    # cv2.polylines(im,[pts_res],True,(255,255,0),2)
                    #
                    # pts = np.array([[int(voc[nqw].split(',')[0]),int(voc[nqw].split(',')[1])],[int(voc[nqw].split(',')[2]),int(voc[nqw].split(',')[3])],[int(voc[nqw].split(',')[4]),int(voc[nqw].split(',')[5])],[int(voc[nqw].split(',')[6]),int(voc[nqw].split(',')[7])],[int(voc[nqw].split(',')[0]),int(voc[nqw].split(',')[1])]], np.int32)
                    # pts = pts.reshape((-1,1,2))
                    # cv2.polylines(im,[pts],True,(255,0,0),2)
                    ...
                    # res_bbx_clockwise = sortpts_clockwise(pts_res)
                    # res_file.write("%d,%d,%d,%d,%d,%d,%d,%d,%s\r\n" % (resres_bbx_clockwise[0][0], resres_bbx_clockwise[0][1], resres_bbx_clockwise[1][0], resres_bbx_clockwise[1][1],
                    #                                                 resres_bbx_clockwise[2][0], resres_bbx_clockwise[2][1],resres_bbx_clockwise[3][0], resres_bbx_clockwise[3][1], qword))
