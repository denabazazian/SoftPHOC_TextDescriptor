...
#accumulator, thetas, rhos = hough_line(binary_binary_query)
binary_im = binary_binary_query.astype(np.uint8) * 255


# Find the connected components from the binarized heatmap of the query
ret, labels = cv2.connectedComponents(binary_im)

# loop over the connected components (background has label 0)
for cur_label in range(len(np.unique(labels))):
    if cur_label == 0:
        continue
    # find all pixels belonging to the current component
    [y_comp, x_comp] = np.where(labels == cur_label)
    # plot the box
    cv2.rectangle(im, (np.min(x_comp), np.min(y_comp)), (np.max(x_comp), np.max(y_comp)), (255, 0, 0), 2)
plt.imshow(im)
plt.show()
...
# lines = cv2.HoughLines(binary_im, 1, np.pi / 180, 100)
# #print len(lines)
...
#         cv2.line(im, (x1, y1), (x2, y2), (0, 255, 0), 8)
