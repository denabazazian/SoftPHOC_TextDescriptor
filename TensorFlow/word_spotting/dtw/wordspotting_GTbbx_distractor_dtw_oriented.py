# -*- coding: utf-8 -*-
"""
Created on Mon Jun 11 2018 19:07:21

@author: dena
"""

# python wordspotting_GTbbx_distractor_dtw_oriented.py /path/to/softPHOC_synthText/word_spotting/data/icdar_ch4_testSet/img_*.jpg
...
if __name__=='__main__':
    ...
    sys.path.append("/softPHOC_tensorflow/softPHOC_utils/")
    ...
    saver.restore(sess, tf.train.latest_checkpoint('/path/to/softPHOC_synthText/tf_model/tf_model_3_backup_118000/'))

    #img_bbx_path = "/path/to/softPHOC_synthText/word_spotting/results/dtw/dtw_GTbbx_GTtranscription_Euclidean_img/"
    res_path = "/softPHOC_synthText/word_spotting/results/dtw/dtw_GTbbx_distractor_Euclidean_res/"
    score_path = "/softPHOC_synthText/word_spotting/results/dtw/dtw_GTbbx_distractor_Euclidean_scores/"
    ...
    for img_name in sys.argv[1:]:
        ...
        scores_file = open(score_path + 'res_' + fileName + '.txt', 'w')
        ...
        for l in range(0, numGTs):
            if (len(gt_file[l])>0 and gt_file[l].split(',')[-1].strip()!='###'):
                ...
                warped_probs, width_gt, height_gt = perspectiveTransfer(test_char_prob[0,...], crd)
                ...
                for nqw in range(0, numQWords):
                    if (len(voc[nqw]) > 0 and voc[nqw].split(',')[-1].strip() != '###'):
                        ...
                        if similarity_type == 0:  # cosine distance
                            ...
                            resized_preds = [warped_probs] ## for warped non axes oriented boxes
                ...
                for s in range(0, len(sorted_idx)):
                    scores_file.write("%d,%d,%d,%d,%d,%d,%d,%d,%s,%.3f\r\n" % (int(gt_file[l].split(',')[0]), int(gt_file[l].split(',')[1]), int(gt_file[l].split(',')[2]), 
                                                                   int(gt_file[l].split(',')[3]), int(gt_file[l].split(',')[4]), int(gt_file[l].split(',')[5]), 
                                                                   int(gt_file[l].split(',')[6]), int(gt_file[l].split(',')[7]), str(queries[s]), float(sims[s])))
                ...
                # pts = np.array([[int(gt_file[l].split(',')[0]), int(gt_file[l].split(',')[1])], [int(gt_file[l].split(',')[2]), int(gt_file[l].split(',')[3])], 
                #     [int(gt_file[l].split(',')[4]), int(gt_file[l].split(',')[5])], [int(gt_file[l].split(',')[6]), int(gt_file[l].split(',')[7])]], np.int32)
                # pts = pts.reshape((-1,1,2))
                # cv2.polylines(img,[pts],True,(0,255,0),2)

                # #cv2.rectangle(img, (resLeft, resTop), (resRight, resBottom), (0, 255, 0), 2)
                # cv2.putText(img, trnscp, (int(gt_file[l].split(',')[0]), int(gt_file[l].split(',')[1])), cv2.FONT_HERSHEY_PLAIN, 1, (0, 255, 0), 2)
