...
# python dtw_houghLines.py data/icdar_ch4_testSet/img_*.jpg
...
def get_houghTransform_points(query,probabilities_np,img_width,img_height ,max_lines = 100,USE_PROBABILITIES = False):
...
if __name__ == '__main__':
    ...
    sys.path.append("./warmspringwinds/models/slim")
        ...
        for image_filename in sys.argv[1:]:
            ...
            #for nqw in range(1, 2): # just for word FUTURE
                if (len(voc[nqw]) > 0 and voc[nqw].split(',')[-1].strip() != '###'):
                    ...
                    st = time.time()
                    houghTransform_proposals = get_houghTransform_points(qword,probabilities_np,img_width,img_height ,max_lines = 100,USE_PROBABILITIES = False)
                        ...
                        similarities = ([np.sum(np.minimum(x, softphoc_query) / np.maximum(x, softphoc_query)) / len(qword) for x in
                         resized_preds])
                            ...
                            for line in houghTransform_proposals:
                                ...
                                pred_points_array = get_points_lines(line, probabilities_np)
