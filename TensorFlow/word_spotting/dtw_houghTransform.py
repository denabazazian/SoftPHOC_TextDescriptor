...
# python dtw_houghTransform.py /dataset/icdar_ch4_testSet/img_*.jpg
...
def get_houghTransform_bbx(query,probabilities_np,img_width,img_height ,max_lines = 3,USE_PROBABILITIES = False):
    ...
    lines = cv2.HoughLinesP(binary_im, 1, np.pi / 180, 100, 100, 10)
    ...
    for l in lines:
        ...
        for x1, y1, x2, y2 in l:           
            ...
            eachBox.append(x1)
            eachBox.append(y1)
        houghTransform_bbx.append(eachBox)

    return houghTransform_bbx    

def get_axis_oriented_proposals(polygone_proposals,logits):

    numProposals = len(polygone_proposals)
    bboxes = np.empty([numProposals, 4]).astype(np.float).astype(np.int)  #some problems casting to int
...
if __name__ == '__main__':
    ...
    img_bbx_path = "./softPHOC_synthText/word_spotting/results/dtw/dtw_houghTransform_img/"
    res_path = "./softPHOC_synthText/word_spotting/results/dtw/dtw_houghTransform_res/"
    ...
    with tf.Session() as sess:
        ...
        saver.restore(sess,"./tf_model/tf_model_3_backup_118000/model_fcn32s_118000.ckpt")
        ...
        for image_filename in sys.argv[1:]:
            ...
            #for nqw in range(1, 2): # just for word FUTURE 
                if (len(voc[nqw]) > 0 and voc[nqw].split(',')[-1].strip() != '###'):
                    ...
                    st = time.time()
                    ### get the polygone proposals for the query from minArea function
                    #polygone_proposals = get_polygoneProposals(qword,probabilities_np,img_width,img_height)
                    ### get bounding box from hough transform lines
                    houghTransform_proposals = get_houghTransform_bbx(qword,probabilities_np,img_width,img_height ,max_lines = 3,USE_PROBABILITIES = False)
                    #if len(polygone_proposals) == 0:
                    #    continue
                    if len(houghTransform_proposals) == 0:
                        continue
                    ### showing the polygone proposals
                    # for l in range(0, len(polygone_proposals)):
                    #     pts = np.array([[int(polygone_proposals[l][2]), int(polygone_proposals[l][3])], [int(polygone_proposals[l][4]), int(polygone_proposals[l][5])], [int(polygone_proposals[l][6]), int(polygone_proposals[l][7])], [int(polygone_proposals[l][0]), int(polygone_proposals[l][1])]], np.int32)
                    ...
                    ### convert the polygone proposals to axis-oriented bounding boxes
                    #bboxes, logitsbb, bbox_widths, bbox_heights = get_axis_oriented_proposals(polygone_proposals,logits)
                    bboxes, logitsbb, bbox_widths, bbox_heights = get_axis_oriented_proposals(houghTransform_proposals,logits)
                    ### showing the bbx 
                    # for l in range(0, len(bboxes)):
                    #     cv2.rectangle(img, (bboxes[l, 0], bboxes[l, 1]), (bboxes[l, 2], bboxes[l, 3]), (255, 0, 0), 2)
                    #     cv2.putText(img, qword, (bboxes[l, 0], bboxes[l, 1]), cv2.FONT_HERSHEY_PLAIN, 1, (255, 0, 0), 2)
                    # plt.imshow(img)
                    # plt.show()

                    #### Remove small proposals
                    # good_boxes_ids = np.where(np.array([np.prod(x) for x in zip(bbox_widths, bbox_heights)]) > min_proposal_area)
                    # bboxes = bboxes[good_boxes_ids]
                    # bbox_widths = bbox_widths[good_boxes_ids]
                    # bbox_heights = bbox_heights[good_boxes_ids]

                    # Crop the heatmaps according to the proposals
                    softphoc_proposal_preds = [probabilities_np[0, b[1]:b[3], b[0]:b[2], :] for b in bboxes.astype(np.int)]
                    num_proposals = len(softphoc_proposal_preds)


                    # Build the soft-phoc representation of the query
                    target_size = (int(len(qword)), 1)  # target size is LEN_QUERY_WORD x 1 x 38
                    ...
                    if similarity_type == 0:  # cosine distance
                        ...
                        [np.sum(np.minimum(x, softphoc_query) / np.maximum(x, softphoc_query)) / len(qword) for x in
                         resized_preds])
                            ...
                            resized_preds = [(cv2.resize(x, (target_size[0], target_size[1]))) for x in softphoc_proposal_preds] ## for axes oriented boxes
                    ...
                    ## since we used the GT vocabulory, we don't need the threshold
                    #if sim > simi_thresh:
                        # if similarities[idx] > simi_thresh:

                    ## this is axis oriented bbx   
                    #res = bboxes[idx, :]
                    # res[4] = similarities[idx]
                    ### resLeft, resTop, resRight, resBottom = [int(x) for x in res[:4]]

                    ### we get the coordinates from polygone proposals with the same idx
                    #res = polygone_proposals[idx, :]
                    #res = polygone_proposals[idx]
                    res = houghTransform_proposals[idx]

                    [bottom_left_X, bottom_left_Y, top_left_X, top_left_Y, top_right_X, top_right_Y, bottom_right_X, bottom_right_Y] = res

                    res_bottom_left_X = int(bottom_left_X)
                    res_bottom_left_Y= int(bottom_left_Y)
                    res_top_left_X = int(top_left_X)
                    res_top_left_Y = int(top_left_Y)
                    res_top_right_X = int(top_right_X)
                    res_top_right_Y = int(top_right_Y)
                    res_bottom_right_X = int(bottom_right_X)
                    res_bottom_right_Y = int(bottom_right_Y)

                    res_file.write("%d,%d,%d,%d,%d,%d,%d,%d,%s\r\n" % (res_top_left_X, res_top_left_Y, res_top_right_X, res_top_right_Y, res_bottom_right_X, res_bottom_right_Y, res_bottom_left_X, res_bottom_left_Y, qword))
                    res_file.flush()
                    ...
                    pts = np.array([[res_top_left_X, res_top_left_Y], [res_top_right_X, res_top_right_Y], [res_bottom_right_X, res_bottom_right_Y], [res_bottom_left_X, res_bottom_left_Y]], np.int32)
                    pts = pts.reshape((-1,1,2))
                    cv2.putText(img, qword, (res_top_left_X, res_top_left_Y), cv2.FONT_HERSHEY_PLAIN, 1, (0, 255, 0), 2)
                    cv2.polylines(img,[pts],True,(0,255,0),2)
